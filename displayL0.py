...
def make_detector_panel(cfg, width=850, height=850,
    ...
    if stepA_points:
        ...
        with open(hits_file, "w") as f:
            f.write(f"Layer,{stepA_name} point,NEVTs,Hits\n")
            for idx, (ptx, pty, nevts) in enumerate(stepA_points, start=1):
                ...
                f.write(f"{TITLE[7]},{idx},{int(nevts)},{hits_str}\n")
    ...
    if stepB_points:
        ...
        print(f"Writing hits to {hits_file}")
        with open(hits_file, "w") as f:
            f.write(f"Layer,{stepB_name} point,NEVTs,Hits\n")
    ...
    if stepC_points:
        ...
        print(f"Writing hits to {hits_file}")
        with open(hits_file, "w") as f:
            f.write(f"Layer,{stepC_name} point,NEVTs,Hits\n")
